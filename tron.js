...
// create an event listener to listen out for those keys being pressed
...
function draw() {
    if (Player.allInstances.filter(p => !p.key).length === 0) {
        ...
        if (playerCount === 1) {
            ...
            outcome = `Player ${alivePlayers[0]._id} win!`;
